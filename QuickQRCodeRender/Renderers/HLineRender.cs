...
{
    public class HLineRender : IDrawQRCode
    {
        public Bitmap DrawQRCode(RenderMatrix matrix, RenderMatrixOptions options)
        {
            ...
            Bitmap lightModulePixel = MakeDotPixel(pixelSize, new SolidBrush(options.LightColor), HLineConverter.Dots.White);
            Bitmap darkModulePixel = MakeDotPixel(pixelSize, new SolidBrush(options.DarkColor), HLineConverter.Dots.Fill);
            Bitmap darkDottedPixel = MakeDotPixel(pixelSize, new SolidBrush(options.DarkColor), HLineConverter.Dots.Dotted);
            Bitmap darkTopPixel = MakeDotPixel(pixelSize, new SolidBrush(options.DarkColor), HLineConverter.Dots.Sx);
            Bitmap darkBottomPixel = MakeDotPixel(pixelSize, new SolidBrush(options.DarkColor), HLineConverter.Dots.Dx);
        ...
        private Bitmap MakeDotPixel(int pixelSize, SolidBrush brush, HLineConverter.Dots dot)
        {
            // draw a dot
            ...
            using (var graphics = Graphics.FromImage(bitmap))
            {
                switch (dot)
                {
                    case HLineConverter.Dots.White:
                    case HLineConverter.Dots.Fill:
                        graphics.FillRectangle(brush, new Rectangle(0, 2, pixelSize, pixelSize-2));
                        graphics.Save();
                        break;

                    case HLineConverter.Dots.Sx:
                        graphics.FillEllipse(brush, new Rectangle(0, 2, pixelSize, pixelSize-2));
                        graphics.FillRectangle(brush, new Rectangle((int)Math.Round(pixelSize / 2f) + 1, 2, (int)Math.Round(pixelSize / 2f) + 1, pixelSize - 2 ));
                        graphics.Save();
                        break;
                    case HLineConverter.Dots.Dx:
                        graphics.FillEllipse(brush, new Rectangle(0, 2, pixelSize , pixelSize-2));
                        graphics.FillRectangle(brush, new Rectangle(0, 2, (int)Math.Round(pixelSize / 2f) + 1, pixelSize - 2));
                        graphics.Save();
                        break;
                    case HLineConverter.Dots.Dotted:
                        graphics.FillEllipse(brush, new Rectangle(0, 2, pixelSize, pixelSize-2));
                        graphics.Save();
                        break;
                } // chiudo switch
    ...
    } // chiudo public class HLineRender : IDrawQRCode
